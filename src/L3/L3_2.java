//2018.09.26, program that finds the specified key in a list with a programmer-declared method which consumes more resource compared to binary search
...
public class L3_2
{
	...
	private static int Search( int[] list, int key, int startIndex )
	{
		...
		{
			for( int counter = startIndex; counter < list.length; counter++ )
			{
				if( list[ counter ] == key )
					return counter;
			} //end for
